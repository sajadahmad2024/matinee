...
| Theme | Material 3, `ColorScheme.fromSeed` per `AppColorScheme`, `AppColors` extension, `AppSpacing` |
...
- **Design tokens go through the theme.** No `Color(`, `TextStyle(` or spacing literal in a widget; a Figma variable maps to a theme role before any widget is written.
