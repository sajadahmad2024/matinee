...
- The bar's 64 is a minimum, not a fixed height: the type scale raised nav labels from 9 to 10, and a user-scaled label has to grow the bar rather than be clipped. Items fill that height so each tap target clears 48dp; the design's own item is 38-43 tall.
