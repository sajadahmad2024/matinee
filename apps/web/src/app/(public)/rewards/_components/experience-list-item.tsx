...
import { type ExperienceReward, type ExperienceStatus, flagEmoji, formatPoints } from "../constants";
...
export function ExperienceListItem({ experience: e }: ExperienceListItemProps) {
  ...
  return (
    <Link href={`/rewards/experience/${e.id}` as Route} className="block">
      <GlassCard
        className={cn(
          "hover:border-primary/50 flex items-center gap-4 border-l-2 p-4 transition-all",
            ...
            {e.status === "live" && (
              <span
                className={cn(
                  ...
                  closingSoon ? "bg-destructive/15 text-destructive" : "bg-warning/15 text-warning",
                )}>
                {hoursToClose <= 0
