...
import { mediaMetadata, gameWidgetConfigs, users, contents, contentSponsorships, contentLicenses, subscriptionPlans, planRegionPrices, subscriptions, subscriptionInvoices, moderationTickets, adminAuditLog, appVersions, appSettings, notificationCampaigns, notificationDeliveries, deviceTokens, userSessions, roles, adminInvites, userMfa, userNotifications, mediaStatusEvents, userEnforcementActions, moderationReports, oauthAccounts, otpCodes, rewardRules, geoPolicies, referralCodes, referralRedemptions, userConsents, rewardCatalogItems, studios, people, contentMedia, rewardRedemptions, contentChangeHistory, contentReactions, comments, commentReports, contentShares, contentModerationLog, contentUnlocks, contentViews, rewardRuleVersions, wallets, quests, userStreaks, ledgerTransactions, predictions, predictionOptions, auctions, bids, predictionEntries, badges, badgeTriggers, contentRegions, rolePermissions, permissions, contentTags, tags, questContents, deviceTokenTopics, contentGenres, genres, contentWatchlist, userBadges, userRoles, commentReactions, questContentProgress, leaderboardMonthly, userMetrics, contentCast, contentProgress, questParticipations, userDailyActivity, contentDailyStats } from "./schema";
...
export const mediaMetadataRelations = relations(mediaMetadata, ({one, many}) => ({
	...
	rewardCatalogItems: many(rewardCatalogItems),
...
export const contentsRelations = relations(contents, ({one, many}) => ({
	...
	auctions: many(auctions),
	predictions: many(predictions),
	contentRegions: many(contentRegions),
...
export const userNotificationsRelations = relations(userNotifications, ({one}) => ({
	...
	mediaMetadatum: one(mediaMetadata, {
		fields: [userNotifications.imageMediaId],
		references: [mediaMetadata.id]
	}),
	notificationCampaign: one(notificationCampaigns, {
		fields: [userNotifications.campaignId],
		references: [notificationCampaigns.id]
	}),
}));
...
export const rewardCatalogItemsRelations = relations(rewardCatalogItems, ({one, many}) => ({
	mediaMetadatum: one(mediaMetadata, {
		fields: [rewardCatalogItems.imageMediaId],
		references: [mediaMetadata.id]
	}),
	user: one(users, {
		fields: [rewardCatalogItems.createdBy],
		references: [users.id]
	}),
	rewardRedemptions: many(rewardRedemptions),
}));
...
export const rewardRedemptionsRelations = relations(rewardRedemptions, ({one}) => ({
	rewardCatalogItem: one(rewardCatalogItems, {
		fields: [rewardRedemptions.itemId],
		references: [rewardCatalogItems.id]
	}),
	user: one(users, {
...
export const contentRegionsRelations = relations(contentRegions, ({one}) => ({
	content: one(contents, {
		fields: [contentRegions.contentId],
		references: [contents.id]
	}),
}));
