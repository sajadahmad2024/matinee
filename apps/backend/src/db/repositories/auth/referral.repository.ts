...
export class ReferralRepository {
  ...
  /** Count of this user's referrals that have completed (qualified or rewarded). */
  async countCompletedByReferrer(userId: string, tx?: DBExecutor): Promise<number> {
    const rows = await this.exec(tx)
      .select({ n: sql<number>`count(*)::int` })
      .from(referralRedemptions)
      .where(and(eq(referralRedemptions.referrerId, userId), inArray(referralRedemptions.status, ['qualified', 'rewarded'])));
    return rows[0]?.n ?? 0;
  }
}
